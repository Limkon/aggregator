...
#define ID_EDT_QUERY        1005// 调整包含顺序：common.h 必须在 gui.h 之前，因为它包含 winsock2.h
// 而 winsock2.h 必须在 windows.h (gui.h 中包含) 之前包含。
#include "common.h"
#include "gui.h"
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        {
            CreateAppFont();
            // 创建浅灰背景画刷
            hBrBkg = CreateSolidBrush(RGB(240, 240, 240)); 
    ...
    // [修复] 处理 Static 控件背景重影问题
    case WM_CTLCOLORSTATIC:
        {
            HDC hdc = (HDC)wParam;
            // 设置文字背景色为窗口背景色 (RGB(240,240,240))
            SetBkColor(hdc, RGB(240, 240, 240)); 
            // 关键：设置为不透明 (OPAQUE)，这样新绘制的文字会覆盖旧内容
            SetBkMode(hdc, OPAQUE);
            return (LRESULT)hBrBkg; // 返回背景画刷
        }
        break;
